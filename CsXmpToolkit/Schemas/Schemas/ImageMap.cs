...
namespace SE.Halligang.CsXmpToolkit.Schemas
{
    ...
    public sealed class ImageMap
    {
        ...
        public ImageMap(Xmp xmp)
        {
            ...
            areas = new XmpArray<Area>(xmp.XmpCore, ns, "Area",
                PropertyFlags.ValueIsArray | PropertyFlags.ArrayIsOrdered,
                new XmpArrayCallback<Area>(AreaCallback)
                );
        }
        ...
        private void AreaCallback(XmpCore xmpCore, string schemaNamespace, string propertyPath,
            PropertyFlags options, XmpArrayCallbackType type, List<Area> items, int itemIndex,
            Area itemValue)
        {
            switch (type)
            {
                case XmpArrayCallbackType.Created:
                    ...
                    string schemaNS, propPath, propValue;
                    ...
                    while (iterator.Next(out schemaNS, out propPath, out propValue, out options))
                    {
                        if (propPath.IndexOf('[') >= 0 && propPath.IndexOf(']') >= 0)
                        {
                            ...
                            xmpCore.GetStructField(schemaNamespace, propPath, Area.Namespace,
                                "type", out shapeTypeString, out options);
                            ShapeType shapeType;
                            ...
                            string fieldPath;
                            XmpUtils.ComposeStructFieldPath(schemaNamespace,
                                propPath, Area.Namespace, "cords", out fieldPath);

                            XmpArray<int> cordsArray = new XmpArray<int>(xmpCore, schemaNamespace,
                                fieldPath, PropertyFlags.ValueIsArray |
                                PropertyFlags.ArrayIsOrdered,
                                new XmpArrayCallback<int>(XmpArray.IntCallback));
                            ...
                            // Read Description
                            XmpUtils.ComposeStructFieldPath(schemaNamespace, propPath, Area.Namespace,
                                "description", out fieldPath);
                            ...
                            // Read Target
                            string target;
                            xmpCore.GetStructField(schemaNamespace, propPath,
                                Area.Namespace, "target", out target, out options);
