...
namespace SE.Halligang.CsXmpToolkit.Schemas
{
    ...
    public sealed class DublinCore
    {
        ...
        {
            xmpCore = xmp.XmpCore;
            common = xmp.CommonFields.DublinCore;

            PropertyFlags options;

            contributor = new XmpArray<string>(xmpCore, ns, "contributor", 
                PropertyFlags.ValueIsArray | PropertyFlags.ArrayIsUnordered, 
                new XmpArrayCallback<string>(XmpArray.StringCallback)
                );

            xmpCore.GetProperty(ns, "coverage", out coverage, out options);

            common.Creator = new XmpArray<string>(xmpCore, ns, "creator", 
                PropertyFlags.ValueIsArray | PropertyFlags.ArrayIsOrdered, 
                new XmpArrayCallback<string>(XmpArray.StringCallback)
                );

            date = new XmpArray<DateTime>(xmpCore, ns, "date", 
                PropertyFlags.ValueIsArray | PropertyFlags.ArrayIsOrdered, 
                new XmpArrayCallback<DateTime>(XmpArray.DateCallback)
                );
            ...
            language = new XmpArray<string>(xmpCore, ns, "language", 
                PropertyFlags.ValueIsArray | PropertyFlags.ArrayIsUnordered, 
                new XmpArrayCallback<string>(XmpArray.StringCallback)
                );

            publisher = new XmpArray<string>(xmpCore, ns, "publisher", 
                PropertyFlags.ValueIsArray | PropertyFlags.ArrayIsUnordered, 
                new XmpArrayCallback<string>(XmpArray.StringCallback)
                );

            relation = new XmpArray<string>(xmpCore, ns, "relation", 
                PropertyFlags.ValueIsArray | PropertyFlags.ArrayIsUnordered, 
                new XmpArrayCallback<string>(XmpArray.StringCallback)
                );
            ...
            common.Subject = new XmpArray<string>(xmpCore, ns, "subject", 
                PropertyFlags.ValueIsArray | PropertyFlags.ArrayIsUnordered, 
                new XmpArrayCallback<string>(XmpArray.StringCallback)
                );
